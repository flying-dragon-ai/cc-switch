...
[farion1231/cc-switch](https://github.com/farion1231/cc-switch) をベースに、上流 v3.11.1 まで同期済み。

**個人利用向けの改造版（実用優先）です。** 追加・変更した機能は十分に検証されておらず、**不具合や上流との非互換が存在する可能性があります**。気になる場合は [公式版](https://github.com/farion1231/cc-switch) の利用を推奨します。

[English](README_EN.md) | [中文](README.md) | 日本語 | [更新履歴](CHANGELOG.md)
...
| 機能 | 公式版 | 個人強化版 |
|------|--------|------------|
| プロバイダカード | 名前のみ表示 | 名前の後ろに現在モデル名を表示 |
| モデルテストボタン | 非表示 | 復元済み |
| テスト用パラメータ設定 | 非表示 | 復元済み（ランダムプロンプトでのテスト対応） |
| トレイ左クリック | メニューを開く | メインウィンドウを開く / 最小化 |
| ワンクリック最前面（最背面） | なし | サイトカード右クリックから利用可能 |
| モデル自動取得 | なし | カスタムドロップダウンで全量表示 + マッチ順ソート |
| モデル分流 | なし | Claude 設定でサイト単位のモデル分流に対応 |
| 新規サイト挿入位置 | 末尾 | 2 番目の位置に挿入 |
...
MiniMax M2.5 は、コーディング・ツール呼び出し・検索・オフィス業務などの主要生産性シナリオで業界 SOTA に到達／更新。アーキテクト級の実装力と高効率なタスク分解能力を備え、前世代比で推論速度は 37% 向上、トークン効率も改善しています。100 token/s で 1 時間連続利用しても約 1 米ドルと低コストで、複雑な Agent の大規模運用を現実的にします。すでに企業の多機能業務シーンで導入が進み、Agent 時代の実運用を加速しています。

[こちらをクリック](https://platform.minimaxi.com/subscribe/coding-plan?code=7kYF2VoaCn&source=link)して MiniMax Coding Plan の限定 12% オフ特典を受け取れます。
...
### 現在のバージョン：v3.11.1 | [完全な更新履歴](CHANGELOG.md) | [リリースノート](docs/release-note-v3.11.1-ja.md)
