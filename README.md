...
| 测试模型按钮   | 已隐藏                    | 已恢复|
| 测试参数配置 | 已隐藏 | 已恢复，并使用随机提示词进行测试的功能                                   |
| 托盘左键单击   | 弹出菜单                  | 打开 / 最小化 主窗口                                |
| 一键置顶 (底) | 无 | 在站点卡片点击右键使用 |
| 自动获取模型   | 无   | 自定义下拉，全量显示 + 匹配排序    |
| 模型分流 | 无 | claude配置 模型可按站点分流 精确控制稳定性与成本
| 新增站点位置 | 排到最后 | 排到第二顺位 (“本宫不死尔等终究是妃” 每个人心都有第一位) |
---
